...
TOKEN = "TOKEN"  # Astra Bot Token (From BotFather)
...
async def start(message: types.Message, state: FSMContext) -> None:
    ...
    if registered:  # Если пользователь зарегистрирован
        ...
        photo = FSInputFile("./Pictures/Banners/Banner1.png")
...
async def lets_go(message: types.message):
    ...
    await asyncio.sleep(2)
    photo = FSInputFile("./Pictures/Banners/Banner2.png")
...
async def whats_next(message: types.message):
    photo = FSInputFile("./Pictures/Banners/Banner3.png")
...
async def intro_end(message: types.message):
    photo = FSInputFile("./Pictures/Banners/Banner4.png")
...
async def unavailable(callback_query: types.CallbackQuery):
    ...
    inkb = InlineKeyboardMarkup(inline_keyboard=[[inb], ])
    photo = FSInputFile(f"./Pictures/Test pictures/{test_num}.png")
...
async def module(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    photo = FSInputFile(f"./Pictures/Test pictures/{callback_query.data.split(' ')[1]}.png")
